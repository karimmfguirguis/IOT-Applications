...
//							}
							...
							for(index =0;index<ipArr.length;index++){
								if(ipArr[index].equals("192.168.210.168")){
									break;
								}
							}
					...
					for(int i =0;i<path.length;i++)
						System.out.print(path[i]+", ");
					int index = 0;
					while (!ipArr[index].equals("192.168.210.168")) {
						index++;
					}
					buffer = msg.getBytes();
					DatagramPacket sendPacket = new DatagramPacket(buffer, buffer.length,
					...
					for(index =0;index<path.length;index++){
						if(path[index].equals("192.168.210.168")){
							break;
						}
					}	
					// check if I am destination to initiate reply to sender
